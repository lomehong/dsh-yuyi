...
export default class YuyiRuntime extends TypertRemoteService {
  ...
  private taskAnchorSession(taskId: string): SessionId | undefined {
    try {
      ...
      if (attach !== undefined) candidates.push(attach.sessionID as SessionId)
      const { events } = readTask(taskId)
      for (let i = events.length - 1; i >= 0; i--) {
        const ev = events[i]!
        if (ev.kind === 'request' && ev.from.sessionID !== undefined) {
          candidates.push(ev.from.sessionID as SessionId)
          break
        }
      }
      const created = events.find((e): e is Extract<TaskEvent, { kind: 'created' }> => e.kind === 'created')
      if (created?.owner.sessionID !== undefined) candidates.push(created.owner.sessionID as SessionId)
      if (candidates.length === 0) return undefined
      for (const candidate of candidates) {
        if (this.ctx.agents.get(candidate) !== undefined) return candidate
      }
      return candidates[0]!
    } catch {
      return undefined
