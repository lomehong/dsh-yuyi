 ...
 */
function ensureCodecFactory(codec: { mode?: string; schema?: unknown; create?: unknown }): void {
...
// descriptor.result 本身就是 codec（result.mode / result.typeSymbol /
// result.schema / result.create 直接平铺）；parameters[].codec 与
// invocation.codec 才是嵌套一层——两种形态并存，都必须覆盖。
for (const descriptor of TYPERT_REMOTE.descriptors) {
  const result = descriptor.result as { mode?: string; schema?: unknown; create?: unknown } | undefined
  if (result) ensureCodecFactory(result)
  if (Array.isArray(descriptor.parameters)) {
    for (const parameter of descriptor.parameters) {
      const codec = (parameter as { codec?: { mode?: string; schema?: unknown; create?: unknown } } | undefined)?.codec
      if (codec) ensureCodecFactory(codec)
    }
  }
  const invocation = descriptor.invocation as { codec?: { mode?: string; schema?: unknown; create?: unknown } } | undefined
