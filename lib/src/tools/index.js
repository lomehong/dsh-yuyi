 ...
 */
import { applyMessagingTools } from "./messaging.js";
import { applyTaskTools } from "./tasks.js";
export { renderStatus, renderPeers, renderInbox } from "./messaging.js";
export { outcomeOf } from "./task-record.js";
...
export function apply(ctx) {
    ...
    ctx.systemPrompt.section({
        ...
        text: 'The yuyi_* tools reach other agents through the hub relay. Address peers by their '
            ...
            + 'Register this session (yuyi_register) before expecting wake deliveries: a delivery '
            + 'carrying a taskId lands on the task-anchored session (the latest yuyi_task_attach '
            + 'wins) even when addressed to a specific session id; without a usable anchor, an '
            + 'agent-level delivery lands on the earliest registered session; live sessions '
            + 'are only picked dynamically as the last resort. A woken session auto-acknowledges '
