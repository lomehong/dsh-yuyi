 ...
 */
import { appendTaskRecord, readTask, } from "../core.js";
/**
  * 把一个追加结果转译为规范值。
  * @param result - 核心追加结果。
  * @returns 规范的记录结果。
...
/**
  ...
  * @returns 请求追加结果。
 */
export function recordTaskRequest(taskId, identity, message, text) {
    const results = [];
 ...
 */
export function recordTaskReply(taskId, reply) {
    if (readTask(taskId).events.length === 0)
        return undefined;
    const from = {
