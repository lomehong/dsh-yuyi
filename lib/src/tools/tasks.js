...
import { appendTaskRecord, archiveTask, closeTask, compactTask, TASK_ID_RE, taskSnapshot, taskView, } from "../core.js";
import { outcomeOf, recordTaskReply, recordTaskRequest } from "./task-record.js";
...
function continueTarget(view, explicit) {
    if (explicit !== undefined && explicit.length > 0)
        return explicit;
    if (view === undefined)
        throw new Error('yuyi_task_continue: the task has no local record; send yuyi_send with task_id first');
    if (view.pendingTarget !== undefined)
        return view.pendingTarget;
...
function registerResultTool(ctx, options) {
 ...
 */
export function applyTaskTools(ctx) {
    const yuyi = ctx.yuyi;
    ctx.tools.register(defineTool({
    ...
    ctx.tools.register(defineTool({
        ...
        output: {
            ...
            render: (_args, value) => [{
                    ...
                    text: value.replyText !== undefined
                        ? `round ${value.messageId} to ${value.to}; reply from ${value.replyFrom}:\n${value.replyText}`
        ...
        async execute(args, exec) {
            ...
            const records = [outcomeOf(appendTaskRecord(args.task_id, attachEvent(yuyi, agent)))];
            ...
            if (replyOutcome !== undefined)
                records.push(replyOutcome);
