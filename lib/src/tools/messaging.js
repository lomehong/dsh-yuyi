...
import { recordTaskReply, recordTaskRequest } from "./task-record.js";
/* * 解析调用方 agent 的会话 id，否则以工具承诺的契约失败。 */
function sessionOf(agent, tool) {
    if (agent === undefined) {
...
export function renderStatus(value) {
    ...
    if (value.agentName !== undefined)
        identity.push(`agent ${value.agentName}`);
    if (value.ownerUsername !== undefined)
        identity.push(`owner ${value.ownerUsername}`);
    if (value.role !== undefined)
        identity.push(`role ${value.role}`);
    const lines = [
...
/**
  ...
  * @returns 渲染出的文本。
 */
export function renderPeers(value) {
    if (value.devices.length === 0)
        return 'no devices are reachable through the hub';
    const lines = [];
...
/**
  ...
  * @returns 渲染出的文本。
 */
export function renderInbox(value) {
    if (value.entries.length === 0)
        return `${value.target} inbox is empty`;
 ...
 */
export function applyMessagingTools(ctx) {
    const yuyi = ctx.yuyi;
    ...
    ctx.tools.register(defineTool({
        ...
        output: {
            ...
            render: (_args, value) => [{ type: 'text', text: renderPeers(value) }],
        },
        async execute() {
    ...
    ctx.tools.register(defineTool({
        ...
        output: {
            ...
            render: (_args, value) => [{ type: 'text', text: renderInbox(value) }],
        },
        async execute(args, exec) {
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            const request = {
                to: args.to,
                text: args.text,
                mode: args.mode === 'mail' ? 'mail' : 'notify',
