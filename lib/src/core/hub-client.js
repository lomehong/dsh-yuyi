import { INBOX_FETCH_LIMIT, PROTOCOL_VERSION, newID, parseFrame } from "./protocol.js";
/** 请求默认超时 */
...
export class HubClient {
    ...
    async taskFetch(taskId) {
        if (!this.supports("task"))
            throw new Error("hub 不支持 task 能力");
        const id = newID("req");
    ...
    maybeReportHeartbeatStats() {
        if (this.heartbeatOk + this.heartbeatFail < HEARTBEAT_STATS_INTERVAL)
            return;
    ...
    handleFrame(frame) {
        switch (frame.type) {
            case "welcome": {
                ...
                this.opts.log?.(`hub welcome: protocolVersion=${this.hubProtocolVersion} features=[${this.hubFeatures.join(",")}]${this.agentName ? ` agent=${this.agentName}` : ""}`);
