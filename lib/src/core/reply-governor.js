 ...
 */
import { REPLY_RATE_PER_SESSION_PER_MIN, REPLY_RATE_INSTANCE_PER_MIN } from "./reply-loop.js";
export { REPLY_RATE_PER_SESSION_PER_MIN, REPLY_RATE_INSTANCE_PER_MIN, REPLY_MAX_ROUNDS_PER_TASK, REPLY_QUEUE_TIMEOUT_NOTIFY_MS, REPLY_RATE_RETRY_MS, REPLY_PENDING_TIMEOUT_MS, REPLY_GRACE_MS, REPLY_RETENTION_MS, } from "./reply-loop.js";
/** 共享限流状态机。每个适配器持有一个实例。单进程单线程（Bun），无需锁。 */
export class ReplyGovernor {
    sessionInjectTimes = new Map();
    instanceInjectTimes = [];
    ratePerSession;
    rateInstance;
    constructor(opts = {}) {
