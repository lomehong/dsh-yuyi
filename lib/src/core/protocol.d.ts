...
export declare const INBOX_FETCH_LIMIT = 50;
...
export declare function recipientID(agentId: string, name: string): string;
/** 送达语义 */
...
export declare function stripHubAuthoritativeFields(message: YuyiMessage): YuyiMessage;
...
export type ClientFrame = HelloFrame | RosterFrame | SendFrame | PeersRequestFrame | AckFrame | InboxFetchFrame | InboxAckFrame | TaskFetchFrame | TraceFrame | HeartbeatStatsFrame;
...
export type HubFrame = WelcomeFrame | ErrorFrame | DeliverFrame | AckFrame | PeersResponseFrame | InboxDataFrame | TaskDataFrame;
/**
 * 解析地址字符串（统一智能体名称寻址）：
 *   "*"                     -> 广播
...
export declare function matchSession(session: RosterSession, target: string): boolean;
/** 生成消息/请求 ID */
export declare function newID(prefix?: string): string;
/** 安全解析一条 JSON 帧 */
export declare function parseFrame<T>(raw: unknown): T | undefined;
