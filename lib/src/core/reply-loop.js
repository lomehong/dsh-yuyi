...
import { ReplyGovernor } from "./reply-governor.js";
import { join } from "node:path";
import { appendTaskRecord, latestAttach, readTask, taskFilePath, taskView, closeTask } from "./yuyi-task.js";
// ---------- 常量（v0.8 §5.1/§5.2） ----------
...
export class ReplyLoop {
    ...
    clearStalePending(maxAgeMs = REPLY_PENDING_TIMEOUT_MS * 2) {
        ...
        if (stale.length > 0) {
            ...
            for (const tid of stale) {
                try {
                    closeTask(tid, "omp", "超时未回复自动关闭（TTL 90min）");
                }
                catch (e) {
    ...
    loadState() {
        try {
            const t = JSON.parse(readFileSync(this.taskFile, "utf8"));
...
export function formatExternalMessage(msg, extraNote = "", signatureVerification) {
