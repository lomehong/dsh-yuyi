...
/**
  ...
  * @returns 注入的文本。
 */
export function formatIncoming(message) {
    const parts = [`[yuyi] from ${senderLabel(message)}`];
    if (message.from.ownerUsername !== undefined)
        parts.push(`owner ${message.from.ownerUsername}`);
    if (message.from.role !== undefined)
        parts.push(`role ${message.from.role}`);
    if (message.replyTo !== undefined)
        parts.push(`reply-to ${message.replyTo}`);
    if (message.taskId !== undefined)
        parts.push(`task ${message.taskId}`);
    return `${parts.join(' · ')}\n${message.text}`;
}
/**
  * 供消息来源元数据使用的一行投递摘要。
  ...
  * @returns 摘要字符串。
 */
export function deliverySummary(message) {
 ...
 * @returns 可投递的回信地址。
 */
export function replyAddressOf(message, selfDevice, selfOwner) {
    const bare = message.from.name ?? message.from.sessionID;
    let address = bare;
    if (message.from.device.length > 0 && message.from.device !== selfDevice)
        address = `${message.from.device}:${bare}`;
 ...
 */
export function collectAssistantText(events, baselineSeq) {
    let text = '';
    for (const event of events) {
        if (event.seq < baselineSeq || event.type !== 'assistant/message')
            continue;
